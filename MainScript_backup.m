...
%% (3) READ/PREPARE DATA: WORDS (ENG/HEB), CHECKERS AND FALSEFONTS
...
sd_data.CNI.rmroiFname = 'rmroicell_CNI_subInds-1-20_dtNames-CB-WE-FF_ROI-all_fits-fFit_2025.mat';
sd_data.ISRAEL.rmroiFname = 'rmroicell_ISRAEL_subInds-31-36-38-44_dtNames-CB-WE-WH_ROI-all_fits-fFit_2025.mat';

for ns = 1:length(fields(sd_data))   
    site_names = fields(sd_data);
    site = site_names{ns};
    disp(site)
    % File name we want with the main data
    fpath = fullfile(sdRP,'DATA',sd_data.(site).rmroiFname);
    read_download_data(fpath)
end
